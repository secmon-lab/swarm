...
func TestClient_Query(t *testing.T) {
	client, err := policy.New(policy.WithPolicyData("test.rego", examplePolicy))
	gt.NoError(t, err)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			err := client.Query(ctx, "data.test", test.input, test.output)
			gt.NoError(t, err)
...
func TestClient_New_WithFile(t *testing.T) {
	...
	err = client.Query(ctx, "data.test", input, &output)
	gt.NoError(t, err)
...
func TestClient_New_WithDir(t *testing.T) {
	...
	client, err := policy.New(policy.WithDir(policyDir))
	gt.NoError(t, err)
...
func TestClient_Query_NoResult(t *testing.T) {
	...
	var output bool
	err = client.Query(ctx, "data", input, &output)
	gt.Error(t, err)
}
